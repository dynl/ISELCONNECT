...
function LinemanReportTab() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const { count: systemTotalCount } = await supabase
        .from("reports")
  ...
  // ==========================================
  // SEPARATE RESOLVED REPORTS SCREEN
  // ==========================================
  if (showResolvedScreen) {
    return (
      <div className="tab-content lineman-dashboard-layout l-rt-tab" style={{ minHeight: "100vh", backgroundColor: "#f8fafc" }}>
        
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "25px", paddingTop: "10px" }}>
          <button 
            onClick={() => setShowResolvedScreen(false)}
            style={{ background: "#ffffff", border: "none", borderRadius: "50%", width: "40px", height: "40px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}
          >
            <ChevronLeft size={24} color="#1b0b8c" strokeWidth={3} />
          </button>
          <h2 style={{ margin: 0, color: "#1b0b8c", fontWeight: "900", fontSize: "1.4rem" }}>RESOLVED REPORTS</h2>
        </div>

        <div className="l-rt-list-wrapper" style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {resolvedReports.length === 0 ? (
            ...
            resolvedReports.map((report) => (
              ...
              >
                <div style={{ flex: 1 }}>
                  <h3 className="lineman-report-title">{report.report_types?.name}</h3>
                  <p className="lineman-report-address">{formatAddress(report)}</p>
                </div>
                <div style={{
                  backgroundColor: "#ecfdf5", color: "#065f46", border: "1px solid #bbf7d0",
                  padding: "6px 12px", borderRadius: "50px", fontSize: "0.7rem", fontWeight: "900", letterSpacing: "0.5px", whiteSpace: "nowrap"
                }}>
                  RESOLVED
                </div>
              </div>
  ...
  // ==========================================
  return (
    <div className="tab-content lineman-dashboard-layout l-rt-tab">
      <div className="l-rt-sticky-header">
        <p className="l-rt-greeting">Hello</p>
        <h2 className="l-rt-name">{linemanName || "Lineman"}</h2>
      </div>

      <div className="l-rt-stats-row">
        <div className="l-rt-stat-box-blue">
        ...
        <div className="l-rt-stat-box-yellow">
      ...
      <button 
        ...
        style={{
          width: "100%", padding: "14px", backgroundColor: "#16a34a", color: "#ffffff",
          borderRadius: "15px", border: "none", fontWeight: "900", fontSize: "0.95rem",
          display: "flex", justifyContent: "center", alignItems: "center", gap: "8px", 
          marginTop: "15px", marginBottom: "25px", boxShadow: "0 6px 15px rgba(22, 163, 74, 0.2)", cursor: "pointer"
        }}
      >
        <CheckCircle size={20} />
      ...
      {/* FIXED FILTER PILLS: 
        Switched to flex-wrap to prevent the clipping box error, 
        and forced bottom margins. 
      */}
      <div style={{ 
        display: "flex", 
        flexWrap: "wrap", 
        ...
        {["ALL", "PENDING", "IN PROGRESS"].map(status => (
          <button
            ...
            style={{
              ...
              fontWeight: "900", fontSize: "0.75rem", cursor: "pointer", whiteSpace: "nowrap",
              backgroundColor: filterStatus === status ? "#1b0b8c" : "#e2e8f0",
              color: filterStatus === status ? "#ffffff" : "#475569",
              transition: "all 0.2s",
              boxShadow: filterStatus === status ? "0 4px 10px rgba(27,11,140,0.2)" : "none"
            }}
          >
            {status}
          </button>
      ...
      {/* FIXED LIST WRAPPER: 
        Forced display flex column to override any negative CSS margins from classNames 
      */}
      <div className="l-rt-list-wrapper" style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "0" }}>
        {loading ? (
          <p className="l-rt-loading">Loading assignments...</p>
        ) : filteredActiveReports.length === 0 ? (
          <p className="l-rt-loading">
            No active reports match this filter.
          </p>
        ) : (
          filteredActiveReports.map((report) => {
            const statusName = report.report_statuses?.name?.toUpperCase() || "UNKNOWN";
            let badgeBg = "#f1f5f9"; 
            ...
            return (
              <div
                ...
                className="lineman-report-card cursor-pointer"
                onClick={() => setSelectedReport(report)}
                style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "10px", margin: 0 }}
              >
                <div style={{ flex: 1 }}>
      ...
      <h2 className="l-rt-section-title text-navy" style={{ marginTop: "30px" }}>GENERAL REPORTS</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "10px" }}>
        {allReports.map((report) => (
          <div key={`general-${report.id}`} className="lineman-report-card" style={{ margin: 0 }}>
            <h3 className="lineman-report-title">
              {report.report_types?.name}
            </h3>
            <p className="lineman-report-address">{formatAddress(report)}</p>
